...
(d) **Steady-state density off the half-fill prediction at large L.** Mean-field gives ρ = 0.5 at α = β = 0.5; we measure 0.58–0.66. Two effects: (i) the measurement window `n_measure = min(5L, 50000)` is far below the relaxation time `~L²` for the larger L (only ~10⁴ steps at L=5000 versus a relaxation time ~10⁷), so the system has not equilibrated; (ii) the central-L/4 slice at [src/fortran/simulation.f90:67-69](../src/fortran/simulation.f90#L67-L69) is still inside the boundary layer at small L. This is a measurement-window issue, not a code bug, and will be revisited when we have an MPI version that can run long histories without storing them all.
